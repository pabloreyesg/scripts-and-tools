...
dcm2niix -f %p $subject

mkdir -p $folder/$subject/DTI_NX

cd -p $folder/$subject/DTI_NX
...
çdsi_studio --action=atk --source=$folder/$subject/DTI_NX/bedpostxfolder/*.fib.gz --tractography_atlas=4 --track_id=corticospinal
